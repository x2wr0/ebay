...
from . import url_bulkexchange, url_filetransfer, api_version, side_id
...
class Connection(BaseConnection):
    ...
    *[ReviseFixedPriceItem]*
    ...
    def __init__(self, **kwargs):
        ...
        self.site_id = self.config.get('site_id', side_id)
        self.execute('getJobs')
        self._jobProfile = self.response.reply.jobProfile
        self._jobs_created = []
        for profile in self._jobProfile:
    ...
    def _get_jobs_created(self):
        ...
        if len(self._jobs_created) > 0:
            jobs = []
            for index in self._jobs_created:
                job = self._jobProfile[index]
                jobs.append({'jobType': job.get('jobType'), 'jobId': job.get('jobId'),
                             'fileReferenceId': job.get('inputFileReferenceId'),
